...
func FindRectancularIntersection(rec1, rec2 map[string]int) (map[string]int, error) {

	leftX, width, err := findRangeOverlap(rec1["leftX"], rec1["width"], rec2["leftX"], rec2["width"])
	if err != nil {
		return map[string]int{}, err
	}
	bottomY, height, err := findRangeOverlap(rec1["bottomY"], rec1["height"], rec2["bottomY"], rec2["height"])
	if err != nil {
		return map[string]int{}, err
	}
	return map[string]int{"leftX": leftX, "bottomY": bottomY, "width": width, "height": height}, nil

}

func findRangeOverlap(point1, length1, point2, length2 int) (int, int, error) {
	//highest point = left/lower side overlap = x to the farthest right or uppermost y
	highest := point1
	if point2 > highest {
		highest = point2
	}
